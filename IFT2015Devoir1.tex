...
public class MistFonction1{
  ...
  pour valeur \texttt{1}, alors que l'autre retourne \texttt{0} 
  sous la condition
  que \textit{l'un des deux} arguments a pour valur \texttt{0}  . 
  \vspace{-1em}
  \paragraph{}
  La seconde portion de la fonction engendre des appels récursifs 
  qui prennent fin lorsque les cas de bases sont rencontrés. Nous constatons 
...
public interface SpecialStack<E> {
  ...
  \begin{algorithm}[H]
      \SetAlgoLined
      \KwResult{Déplacer tous les éléments de Nam à Sam}
     initialisation\;
     ArrayStack nam, pam, sam\;
     nam.push("N5: Le Roi")\;
     nam.push("N4: Gouvernement")\;
     nam.push("N3: Académie")\;
     nam.push("N2: Manufacture")\;
     nam.push("N1: Agriculture")\;
     \While{nam n'est pas vide OU pam n'est pas vide}{
      \eIf{nam n'est pas vide}{
       déplacer le sommet de nam à pam\;
       }{
       déplacer le sommet de pam à sam\;
      }
      imprimer l'état des piles\;
      incrémenter le compteur de jours\;
      \If{compteur de jours est égal à 10}{
       sortir de la boucle\;
      }
     }
     \eIf{sam contient tous les éléments}{
      afficher "Déplacement possible en X jours"\;
      }{
      afficher "Déplacement non possible en 10 jours"\;
     }
     \caption{Algorithme de déplacement de Nam à Sam}
  \end{algorithm}
...
\end{lstlisting}
    ...
    \section{Algorithme et pseudo-code}
        ...
        \begin{algorithm}[H]
        \DontPrintSemicolon
        \KwData{\texttt{sequenceDeMots}, un tableau de mots}
        \KwResult{Modifie sequenceDeMots en supprimant les éléments redondants consécutifs}

        \SetKwFunction{FMain}{pairDestroyer}
        \SetKwProg{Fn}{Function}{:}{}
        \Fn{\FMain{sequenceDeMots}}{
            \If{sequenceDeMots est vide}{
                \KwRet\;
            }
            tPile $\gets$ une nouvelle instance de \texttt{ArrayStack}\;
            \ForEach{\texttt{mot} dans \texttt{sequenceDeMots}}{
                \eIf{tPile n'est pas vide et tPile.top() est égal à mot}{
                  \texttt{ tPile.pop()}\;
                }{
                  \texttt{tPile.push(mot)}\;
                }
            }
            Afficher("Nombre de mots après suppression : " + \texttt{tPile.size()})\;
        }
